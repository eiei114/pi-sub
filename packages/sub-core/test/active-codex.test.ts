...
for (const lateFailure of [false, true]) {
	test(`same-alias late authentication ${lateFailure ? "failure" : "success"} cannot replace newer quota`, async () => {
		const { deps } = createDeps({ fetch: async () => response(70) });
		const active = createActiveCodexUsage(deps);
		const older = ctx("openai-codex", "old");
		const original = older.modelRegistry.getApiKeyAndHeaders;
		let release!: () => void;
		older.modelRegistry.getApiKeyAndHeaders = async (model) => {
			await new Promise<void>((resolve) => { release = resolve; });
			if (lateFailure) throw new Error("old authentication failed");
			return original(model);
		};
		const slow = active.refresh(older as never, 0, 0, () => {});
		await active.refresh(ctx("openai-codex", "new") as never, 0, 0, () => {});
		const latest = active.usage;
		release();
		await slow;
		assert.equal(active.usage, latest);
		assert.equal(active.usage?.windows[0].usedPercent, 70);
	});
}

test("old quota response cannot restore quota after newer same-alias authentication fails", async () => {
	let release!: (value: Response) => void;
	const { deps } = createDeps({ fetch: async () => new Promise<Response>((resolve) => { release = resolve; }) });
	const active = createActiveCodexUsage(deps);
	const slow = active.refresh(ctx("openai-codex") as never, 0, 0, () => {});
	await new Promise((resolve) => setImmediate(resolve));
	const failed = ctx("openai-codex");
	failed.modelRegistry.getApiKeyAndHeaders = async () => { throw new Error("auth failure"); };
	await active.refresh(failed as never, 0, 0, () => {});
	assert.ok(active.usage?.error);
	release(response(10));
	await slow;
	assert.ok(active.usage?.error);
