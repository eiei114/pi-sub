...
> **Unofficial provider APIs:** Cursor, OpenCode, Command Code, xAI (Grok), and Ollama Cloud usage endpoints are unofficial and may change without notice. Failures soft-error in the UI; disable the provider in settings if an API shape breaks.
